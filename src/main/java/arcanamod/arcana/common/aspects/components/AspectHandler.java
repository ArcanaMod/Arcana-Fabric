...
public interface AspectHandler {
	...
	default AspectHolder findFirstHolderContaining(@NotNull Aspect aspect){
		return findFirstHolderMatching(holder -> holder.getStack().getAspect() == aspect);
	}
	/** Returns all holders containing a particular aspect. */
	default List<AspectHolder> getAllHoldersContaining(@NotNull Aspect aspect){
	...
	default Stream<AspectHolder> streamAllHoldersContaining(@NotNull Aspect aspect){
		return getHolders().stream()
				.filter(holder -> holder.getStack().getAspect() == aspect);
	}
	...
	*/
	...
	/**
	 ...
	 Parts of the input stack may be inserted into different holders.
	 */
	default float insert(@NotNull Aspect in, float amount){
		float transferred = 0;
		if(amount > 0){
	...
	*/
	...
	/**
	 ...
	 Holders will be drained in index order. Multiple holders may be drained.
	 */
	default float drain(@NotNull Aspect aspect, float amount){
		float transferred = 0;
		if(amount > 0)
			for(AspectHolder holder : getHolders()){
				if(amount == 0)
