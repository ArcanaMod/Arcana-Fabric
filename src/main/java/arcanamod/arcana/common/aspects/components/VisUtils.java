...
public final class VisUtils{
	 ...
	 */
	public static void moveAllAspects(@NotNull AspectHandler from, @NotNull AspectHandler to, float max){
	...
	public static void moveAspects(@NotNull Aspect in, float amount, AspectHandler to, float max){
		int transferred = 0;
		if(amount > 0){
			List<AspectHolder> holders = to.getHolders();
			// move void cells, then empty cells to the end
